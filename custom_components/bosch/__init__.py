...
# HA companion app's in-app browser). On iOS Safari opened as a regular tab
# it won't close, so we provide the homeassistant:// deep link.
_OAUTH_CALLBACK_SUCCESS_HTML = """<!DOCTYPE html>
<html><head><title>Bosch Login</title></head>
<body>
<script>
    window.close();
    setTimeout(function() {
        window.location.href = 'homeassistant://navigate/config/integrations';
    }, 300);
</script>
<p>Login successful! Returning to Home Assistant&hellip;</p>
<p><a href="homeassistant://navigate/config/integrations">
   Tap here if not redirected automatically
</a></p>
</body></html>"""

_OAUTH_CALLBACK_ERROR_HTML = """<!DOCTYPE html>
<html><head><title>Bosch Login Failed</title></head>
<body>
<p>Login failed. Please return to Home Assistant and try again.</p>
<p><a href="homeassistant://navigate/config/integrations">Return to Home Assistant</a></p>
</body></html>"""


class BoschOAuthCallbackView(HomeAssistantView):
    """Captures the OAuth2 redirect from Bosch SingleKey ID.

    HA registers this at /api/bosch_easycontrol/callback.
    The config flow uses this URL as redirect_uri so the browser
    lands here after login instead of a dead mobile app scheme.
    """

    url = OAUTH_CALLBACK_PATH
    name = "api:bosch_easycontrol:callback"
    requires_auth = False  # must be public — browser arrives unauthenticated

    def __init__(self, hass: HomeAssistant) -> None:
        self.hass = hass

    async def get(self, request: web.Request) -> web.Response:
        """Handle OAuth callback: resume the config flow with the auth code.

        The OAuth server echoes back whatever 'state' we sent in the auth
        request — we use flow_id as state, so we can always find the right flow.
        """
        code = request.query.get("code")
        flow_id = request.query.get("state")  # echoed back by OAuth server
        error = request.query.get("error")

        if not flow_id:
            return web.Response(status=400, text="Missing state/flow_id parameter")

        try:
            await self.hass.config_entries.flow.async_configure(
                flow_id,
                user_input={"code": code, "error": error},
            )
        except Exception as err:
            _LOGGER.error("OAuth callback failed for flow %s: %s", flow_id, err)
            return web.Response(
                status=200,
                content_type="text/html",
                text=_OAUTH_CALLBACK_ERROR_HTML,
            )

        return web.Response(
            status=200,
            content_type="text/html",
            text=_OAUTH_CALLBACK_SUCCESS_HTML,
        )
...
async def async_setup(hass: HomeAssistant, config: ConfigType):
    ...
    hass.data[DOMAIN] = {}
    hass.http.register_view(BoschOAuthCallbackView(hass))
    return True
...
class BoschGatewayEntry:
    ...
    async def async_init_bosch(self) -> bool:
        ...
        try:
            ...
            _LOGGER.warning("OAuth tokens expired, triggering re-authentication: %s", err)
            self.hass.async_create_task(
                self.hass.config_entries.async_start_reauth(self.config_entry)
            )
            return False
    ...
    def _persist_tokens_if_changed(self) -> None:
        """Write refreshed OAuth tokens back to the config entry.

        No-op for non-OAuth gateways (those don't have tokens_changed()).
        Called after every periodic refresh so tokens are always persisted
        immediately after an in-memory refresh, surviving HA restarts.
        """
        ...
        if self.gateway.tokens_changed(
            ...
            _LOGGER.debug("OAuth tokens refreshed, persisting to config entry")
