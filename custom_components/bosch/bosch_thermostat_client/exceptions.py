...
class TokenExpiredException(BoschException):
    """OAuth refresh token is expired or invalid; re-authentication required.

    NOT a subclass of DeviceException so it is never swallowed by
    _update_info()'s broad ``except DeviceException: pass`` handler.
    """
    pass
